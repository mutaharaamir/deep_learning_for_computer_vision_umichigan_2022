...
def multiples_of_ten(start: int, stop: int) -> Tensor:
    ...
    Args:
        start: Beginning of range to create.
    ...
    # x = torch.arange(start//10 * 10, stop//10 * 10 + 1, 10, dtype=torch.float64)
    values = [i for i in range(start, stop) if i % 10 == 0]
...
def reverse_rows(x: Tensor) -> Tensor:
    ...
    idx = reversed(torch.arange(x.shape[0]))
    y = x[idx]
...
def make_one_hot(x: List[int]) -> Tensor:
    ...
    l = len(x)
    y = torch.zeros((l, 1+max(x)))
    y[torch.arange(l), x] = 1
...
def reshape_practice(x: Tensor) -> Tensor:
    ...
    y = x.view(6,-1)[[0,3,1,4,2,5]].view(3,8)
...
def normalize_columns(x: Tensor) -> Tensor:
    ...
    mu = torch.sum(x, axis=0)/x.shape[1]
    # 0 division errors
    sigma = torch.sqrt(torch.sum((x - mu)**2, axis=0)/(x.shape[1]-1))
    y = (x - mu)/sigma
